...
date: 2021-10-11T06:47:37.699Z
description: 基于 DrawIO 源码，简单的介绍 DrawIO 的二次开发流程，完成一个团队图表绘制管理系统。
---

![DrawIO_Logo](https://p5.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10854593673/4df8/5af6/fefe/84ea579abbdc46d055ceb619e3dea4e2.png)

> 本文作者：[溪岼](https://github.com/luffyZh)

本文从使用角度入手，循序渐进的讲述 DrawIO 是什么，为什么要基于 DrawIO 进行二次开发以及 DrawIO 二次开发的核心技术细节并抽象出了一套通用二开架构，因此文章内容较长，不同阶段的读者可以根据需求进行阅读。本文整体结构如下：

1 - DrawIO 二次开发需求背景 **（一入 DrawIO 深似海）**

2 - DrawIO 是什么以及使用方法简介 **（揭开 DrawIO 的神秘面纱）**

3 - DrawIO 二次开发的准备阶段 **（山重水复疑无路，老泪纵横的跑通 DrawIO 代码）**

4 - DrawIO 二次开发的技术细节 **（柳暗花明又一村，掌握 DrawIO 核心二开代码）**

5 - DrawIO 二次开发架构详解 **（三省吾身，抽象出通用的 DrawIO 开发架构）**


## 背景

对于绘制各种图表，诸如流程图、原型图、拓扑图、UML 图以及思维导图等等，你是不是还在用微软功能庞大但必须下载客户端同时体积也很庞大的 Visio？抑或是你还在用 ProcessOn 免费版，十张图紧紧巴巴来回替换绘制，画完这张图下载下来删除再重新画另一张（当然如果你是氪金玩家，那么我只能说，大佬开心就好～）？在上述诸多问题的背景之下有了这篇文章，看完过后希望不同读者能够得到相应收获。

- 单纯的使用人员，可以获得一个简单免费无限空间的高级绘图工具。
- 进阶的开发人员，可以为自己和团队非常简单快速的搭建一个免费无限空间且功能强大的绘图工具。
- 高阶的二开人员（了解并尝试过 DrawIO 二开），可以掌握一种新的二次开发模式，逐渐成为捕鱼达人。

扯得这么高大上，弄的都有些不好意思了。重新阐述一下原因就是目前笔者在云音乐主要负责的工作是提升研发线效率，某天产品和后端同学提出了一个需求，大体上意思就是团队需要一个内部图表工具，这个图表工具能和现今公司工作流串联起来完成流程闭环，因为目前的工作模式就是产品/研发用 ProcessOn 或者其他绘图工具绘制图表后，截图/保存到本地然后再进行粘贴，因为 ProcessOn 有空间限制并且也不必所有人都开会员😂，因此还不如干脆就内部搞一个了。

基于上述官方/非官方种种原因，笔者开始着手调研这个内部图表系统的搭建，观望一圈之后最终确认技术选型 —— 以 DrawIO 为基础进行二次开发。

> **【注意】**：本文后面统一将**二次开发**简称为**二开**

目前来说这个绘图系统的基本功能（绘制/存取等）已经开发完成并内测使用中，并且笔者还为网站起了一个高端大气上档次的名字 —— **Overmind-X Graph**，一起来看看网站现阶段效果：

![overmind-x graph drawio](https://p6.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10679522146/f00b/1179/a928/801e87f5c5d0fb9f74b3088791087c67.gif)

![overmind-x graph list](https://p5.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10679521800/279b/a841/beaa/f6d9e5c7d449fe35eccca8621ef4ee3b.gif)
...
这里笔者新建了一个名为 [drawio-folder](https://github.com/luffyZh/drawio-folder) 的仓库，里面只有一个 `README.md` 文件，如下图所示：

![DrawIO Github Folder](https://p6.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10685277405/b401/8216/0dc6/849ecc2b112431553c97e7dc4d8b03f3.jpg)

- 2 - 绘图时选择 GitHub 作为存储空间

然后从官网点击 Start 按钮进入绘图页面，第一次进入页面不出意外会出现一个如下图所示选择存储空间的选项，然后选择 Github（其他远程存储原理一样）。

![DrawIO Github Service](https://p5.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10679612720/eafa/28d3/c577/e1c7e7893a4102741e76b5540ce5394c.png)

- 3 - 选择对应的仓库然后绘制图片

点击选择存储到 Github 仓库，然后选择新建一个图表，命名之后点击新建 Create 按钮完成图表的创建。

![DrawIO Create Graph](https://p5.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10679615009/f144/8e3a/316a/a0e09c81909fe85174dc656dfe80964a.png)

- 4 - 授权选取存储仓库

图标创建成功后，会出现一个授权登录的弹窗，授权登录过后 DrawIO 就能获取到账户下的所有仓库信息，选择刚才创建的 **drawio-folder** 仓库，然后进行图表的绘制。

![DrawIO Select Folder](https://p6.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10685276566/01d6/a7e3/e29c/45e8cc7bcffaeca717c3b819ee9a7d4a.jpg)

- 5 - 绘图完毕保存文档

绘制过程中，每一次的保存操作，都会创建一条 `commit message`，就相当于正常的仓库修改并提交，如下图所示：

![DrawIO Commit Message](https://p6.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10679612721/712c/40b0/c355/1785a17906c1680f2616e7fa7ac0d301.png)

当一次图表绘制完成并保存后，可以去 Github 仓库看看是否存储成功。

![DrawIO Git Result](https://p6.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10685275686/ad11/030d/b3e2/6e70a39a83b52df44114094591c733ac.jpg)
...
![DrawIO Java Ant](https://p5.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680217029/1ddb/ccd0/9067/6028281e3ddc71bcdd6da747b5566105.png)

安装完成之后可以在命令行验证是否安装成功，如果命令行里出现了如上图所示对应的 Java 和 Ant 版本信息，那么恭喜你 DrawIO 二开的基础环境已经准备完成，接下来就是正式进入 DrawIO 二开了。

### DrawIO 源码

二开的开始，首先从 [DrawIO 源码仓库](https://github.com/jgraph/drawio) Clone 官方代码，在本地跑一下看看效果。老规矩 Clone 下来先阅读 `package.json` 文件，发现其中只有一个 `start` 命令。
...
![DrawIO Electron](https://p5.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680214807/ef22/e810/5bb3/0c6730b61633d20c0ca4d681e38e0170.png)

它的使用方式和上面文章里介绍的一样，但是问题来了！我们最终目标是要进行 DrawIO 二开，难不成是基于 Electron 二开吗？虽然说也不是不行，但是那上手成本可就高了，估计会劝退一大波同学，不要慌，去官方仓库看看有没有其他方法。

![DrawIO Github Pages](https://p6.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680217025/8dbb/8bca/3013/4b75206f4131c8dbdc3c757cebef2a2b.png)
...
![DrawIO Static Site](https://p5.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680214806/085a/13d6/b4ef/f249b749385314f8a3a2433152fb20ec.png)
...
![DrawIO Dev Preview Code](https://p5.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680216696/3c7e/57c5/11a3/3d92062086e647e4fc53efbc524b3784.png)

很容易理解，就是 Alert 一个弹窗提示测试一下代码是不是能正常执行，不过刷新页面之后会发现并没有看到这个弹窗，这是为什么呢？打开控制台找找原因：

![DrawIO MinJs](https://p5.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680217020/2ce0/bae4/8b67/29b79f0979a6d0116a9103eb49268903.png)
...
![DrawIO Dev Param](https://p5.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680214802/29df/a1fd/4e47/3ada001f9b1fc6006aa381c3eb592e01.png)
...
![DrawIO Dev Error Reason](https://p5.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680215711/92d8/65f5/8a05/ca51bcaaf92984005a81b35d9f634987.png)
...
![DrawIO Dev Success](https://p5.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680215723/ceff/c9d0/0da7/553c6a9e7adaf591f782850a7a88e7d9.png)
...
![DrawIO Build Success](https://p6.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680216704/4c5d/6342/6de1/83e8347b33ecfd1dc1f631407f81316a.png)

然后再来访问一下首页，这次不携带 `dev=1` 开发参数看看效果，如下图所示，非常完美的运行着新开发的生产版本，至此为止 DrawIO 的二开流程阶段，从开发预览到构件生产版本都已经介绍完成。

![DrawIO Prod Success](https://p6.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680217028/a642/a55f/6b13/4a0933e4c57f73cdb76661952530086a.png)
...
- [x] 基于云音乐内部服务的增删改查
- [x] 个人中心模块，管理自己的图表内容
- [x] 数据交互，图表与业务系统进行数据交互，一键导入已创建的图表
- [ ] Todo - 鉴权功能，比如分享，指定权限的人可以进行相应操作 
- [ ] Todo - 团队空间，一个团队可以管理一个图表的集合
- [ ] Todo - 版本历史，支持查看最近几次的版本修改内容
- [ ] Todo - 自定义导入一些基于公司/个人比较常用的模板
...
![DrawIO Console File](https://p5.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680336133/5fa9/3664/4265/1276bbd29759a59013c77d0f6b187507.png)
...
![DrawIO FileSaver](https://p5.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680275195/0fdc/18d0/defc/37a11ae3f22dc31342f78a6fcffaa570.gif)
...
![DrawIO LoadFile](https://p6.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10679527923/3e55/737e/6e92/dd0522f48441a4c77800c0f4bdc03fd0.gif)
...
>
> - [个人版 DrawIO](https://simple-drawio-starter.vercel.app/)
> - [DrawIO 文件下载](https://simple-drawio-starter.vercel.app/drawio/filesave/index.html)
> - [DrawIO loadFile 加载文件](https://simple-drawio-starter.vercel.app/drawio?id=26)
...
![DrawIO Static Site Structure](https://p6.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680334371/b2ba/a24d/91d4/96d572200142ce0de066709ca0794020.png)
...
![DrawIO Two System Structure](https://p6.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680334372/e9c2/eb28/b4c3/12452f29465e5e69f72a3d105217728d.png)
...
![DrawIO Special Structure](https://p6.music.126.net/obj/wo3DlcOGw6DClTvDisK1/10680335240/52dd/e5d3/401c/304a2db2de309df5e4d912f68aa74c57.png)
...
那么主系统是通过何种方式完成绘图系统与业务系统的桥接呢？答案就是通过 Next.js 的 [Rewrites](https://nextjs.org/docs/api-reference/next.config.js/rewrites) 特性，感兴趣的可以去查阅官方文档，正是因为这个特性才使得单系统进行 DrawIO 二开变成了可能并且开发起来也非常便利，**桥接双系统让整个工程对外形成一个独立完整系统**算是这个架构的核心之处。
...
> 讲这么多其实都帮各位看官准备好了，基于**架构三**搭建了一个脚手架 [simple-drawio-starter](https://github.com/luffyZh/simple-drawio-starter)，大家直接拿来进行业务逻辑的开发即可。
